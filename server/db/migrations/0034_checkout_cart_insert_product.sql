-- 0034: 0033 backfilled product_id/variant_id in a later UPDATE, but the
-- INSERT aborts on the NOT NULL violation before that runs. Resolve the
-- first cart line's variant/product BEFORE the insert and write both
-- inline. Full per-line validation still happens in the loop below and
...
BEGIN
  ...
  -- Legacy single-line columns, resolved up front so the INSERT below
  -- satisfies the NOT NULL constraint. The loop re-validates every line
  -- (active, vault, stock) and rolls back on any failure.
  SELECT pv.id, pv.product_id
  INTO v_first_variant_id, v_first_product_id
  FROM cart_items ci
  ...
  INSERT INTO orders (
    public_id, user_id, cart_id, product_id, variant_id, amount, status,
    payment_provider, customer_account, wa_number, payment_status,
    fulfillment_status, subtotal, payment_fee, payment_total,
    terms_consented_at
  )
  VALUES (
    v_public_id, p_user_id, p_cart_id, v_first_product_id, v_first_variant_id,
    0, 'PENDING', p_payment_provider,
