...
const EMPTY: PublicSettings = { storeName: '', announcement: '', whatsapp: '', telegram: '', email: '', termsBody: '', termsUpdatedAt: '', paymentMethods: ['manual'], sumopodMinAmount: 10_000, sumopodFeePct: 0.007, sumopodFeeFixed: 300 }
...
async function load(): Promise<PublicSettings> {
  ...
  if (!inflight) {
    inflight = api.api.settings.public
      ...
      .then((j) => ({ ...EMPTY, ...j, paymentMethods: Array.isArray((j as any).paymentMethods) ? (j as any).paymentMethods : EMPTY.paymentMethods, termsBody: typeof (j as any).termsBody === 'string' ? (j as any).termsBody : EMPTY.termsBody, termsUpdatedAt: typeof (j as any).termsUpdatedAt === 'string' ? (j as any).termsUpdatedAt : EMPTY.termsUpdatedAt, sumopodMinAmount: typeof (j as any).sumopodMinAmount === 'number' ? (j as any).sumopodMinAmount : EMPTY.sumopodMinAmount, sumopodFeePct: typeof (j as any).sumopodFeePct === 'number' ? (j as any).sumopodFeePct : EMPTY.sumopodFeePct, sumopodFeeFixed: typeof (j as any).sumopodFeeFixed === 'number' ? (j as any).sumopodFeeFixed : EMPTY.sumopodFeeFixed }))
      .catch(() => EMPTY)
      .finally(() => {
        inflight = null
      })
